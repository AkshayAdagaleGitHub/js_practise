function getShortestUniqueSubstring(arr, str) {
  const countMap = {};
  // arr.forEach(item => countMap[item] = (countMap[item] || 0) + 1);
  arr.forEach(item => countMap[item] = 0 );
  let headIndex = 0;
  let uniqueCounter = 0;
  let result = '';
  ...
  for(let i = 0; i < str.length; i++) {
   if (countMap[str[i]] === 0) {
     countMap[str[i]] = 1;
     uniqueCounter++;
   } else if (countMap[str[i]] >= 1){
     countMap[str[i]]++;
   }
   while (uniqueCounter === arr.length ) {
     result = str.substring(headIndex, i + 1);
     if(countMap[str[headIndex]] !== undefined
         && countMap[str[headIndex]] !== 0){
       countMap[str[headIndex]]--;
       if (countMap[str[headIndex]] === 0) {
         uniqueCounter--;
       }
     }
     headIndex++;
...
console.log(getShortestUniqueSubstring(['a', 'b', 'c'], 'aabcbc'));
